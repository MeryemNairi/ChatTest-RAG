...
async def ask_question(question: Question):
    try:
        prompt = prompts.get("hr_chatbot_prompt", "")
        user_message = question.question
        prompt_with_context = f"{prompt}\n\nUser: {user_message}\nChatbot:"

        # Vérification simple pour les questions hors sujet
        if any(phrase in user_message.lower() for phrase in ["hors sujet", "out of scope"]):
            response = prompts.get("off_topic", "Je suis désolé, mais je ne peux pas aider avec ce sujet.")
        else:
            response = app.state.conversation_chain.invoke({'question': user_message})
